  ...
  <div class="paperDetail">
    ...
    <div class="info">
      <div>
        <label>作者：</label>
        <div class="name">
          <!-- <span class="hight" @click="toExpertDetail(item.id)" v-for="(item, index) in detailData.authors" :key="index">{{item.name}}</span><span v-for="(item, index) in detailData.names" :key="index">{{item}}</span> -->
          <span class="height">{{ detailData.authors }}</span>
        </div>
      </div>
      <p><label>出处：</label><span>{{detailData.journalName}} {{detailData.journalNo}}</span></p>
    </div>
    <div class="summary">
      <h4>摘要：</h4>
      <p class="abstract">{{detailData.paperAbstract || '暂无内容'}}</p>
    </div>
    <div class="relative" v-if="similarData.length">
      <h4>相关论文：</h4>
      <div class="clearfix">
        <span v-for="(item, index) in similarData" v-if="item" :key="index" @click="getAllInfo(item.paperId)">{{item.name}}</span>
      </div>
    </div>
  </div>
</template>
  ...
  .paperDetail {
    ...
    .info{
      ...
      p{
        ...
        label{height: 40px;line-height: 40px;flex-basis: 94px;color: #999;}
