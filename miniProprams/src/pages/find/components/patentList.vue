<template>
  <div class="patentList">
    ...
    <scroll-view scroll-y="true" class="patent_data" :lower-threshold="500" enable-back-to-top="true" :scroll-into-view="scrollTopId" @scrolltolower="scrolltolower">
      ...
      <p class="title" v-if="searchList.docs && searchList.docs.length">已找到<span>{{searchList.numFound}}项</span>专利</p>
      <div class="card_lists" v-if="searchList.docs && searchList.docs.length">
        <div class="card" v-for="(item, index) in searchList.docs" :key="index" @click="toDetail(item.id)">
          ...
          <div class="row-between type_time">
            <span class="type">{{item.patent_type}}</span><span class="time">{{item.publication_date}}</span>
          </div>
    ...
    methods: {
      ...
      async search(isloadMore) {
        ...
        data.docs.forEach((item) => {
          item.patent_type = Filters.filters.patentType(item.patent_type);
      ...
      toDetail(id) {
        this.$router.push('/pages/find/patentDetail?id=' + id);
      },
    },
    watch: {
      'searchPostData.patentType': function() {
        this.search();
      },
      ...
      .card_lists{
        ...
        .card{
          ...
          .type_time{
            ...
            .type{height:43px;line-height: 41px;border-radius:5px;border:1px solid #3576FF;padding: 0 20px;color: #3576FF;text-align: center;font-size: 24px;}
            .time{color: #999;}
          }
          >p{
