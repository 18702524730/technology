<template>
  <div class="paperList">
    ...
    <scroll-view scroll-y="true" class="patent_data" :lower-threshold="500" enable-back-to-top="true" :scroll-into-view="scrollTopId" @scrolltolower="scrolltolower">
      ...
      <p class="title" v-if="searchList.docs && searchList.docs.length">已找到<span>{{searchList.numFound}}项</span>论文</p>
      <div class="card_lists" v-if="searchList.docs && searchList.docs.length">
        <div class="card" v-for="(item, index) in searchList.docs" :key="index" @click="toDetail(item.id)">
          ...
          <div class="row-between type_time">
            <span class="type">{{item.journal_name}}</span><span class="time">{{item.journal_no}}</span>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toDetail(id) {
        this.$router.push('/pages/find/paperDetail?id=' + id);
      },
    },
    watch: {
      'searchPostData.journalQuality': function() {
        this.search();
      },
