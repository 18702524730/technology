<template>
  ...
  >
    <div class="_v-content" :class="{extra: notFirst && loadingState == 0}" :id="contentId">
      ...
      >
        <span class="spinner-holder">
          ...
          <span class="text" v-if="state == 0" :style="{color: refreshLayerColor}" v-text="refreshText"></span>
          <span class="text" v-if="state == 1" :style="{color: refreshLayerColor}" v-text="refreshText1"></span>
      ...
      <div v-if="showInfiniteLayer" class="loading-layer">
        <span class="spinner-holder" :class="{'active': showLoading}">
          <slot name="infinite-spinner">
            <spinner2 :style="{fill: loadingLayerColor, stroke: loadingLayerColor}"></spinner2>
          </slot>
        </span>
...
<style lang="scss">
  ...
  ._v-container > ._v-content .spinner-holder{
    position: absolute;
    top: 50%;
  ...
  ._v-container > ._v-content > .loading-layer .spinner-holder .text {
    background: transparent url(~assets/img/loading.gif) left center no-repeat;background-size: px(90) px(24);padding-left: px(100);font-size: px(24);
    display: inline-block;
  }
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      // 增加容器的初始定位，用于还原真实的最大可滚动高度
      let positionTop = getComputedStyle(this.container, null).top.slice(0,-2)-0 || 0;
      let paddingTop = getComputedStyle(this.container, null).paddingTop.slice(0,-2)-0 || 0;
      ...
      this.scroller = new Scroller(render, {
        ...
        positionTop: positionTop + paddingTop,
				scrollToTopCallback: this.scrollToTopCallback
      })

      // enable PullToRefresh
      if (this.onRefresh) {
        this.scroller.activatePullToRefresh(H, () => {
          this.state = 1
        }, () => {
          this.state = 0
        }, () => {
      ...
      if (this.onInfinite) {
        this.infiniteTimer = setInterval(() => {
          ...
          this.notFirst = true;
          if (top + H > this.content.offsetHeight - this.container.clientHeight) {
    ...
    methods: {
			scrollToTopCallback(){
				this.$emit('update:isFixed', false);
			},
      ...
      touchStart(e) {
        ...
        if (e.target.tagName.match(/input|textarea|select/i)) {
          return
				...
				this.__lastTouchTop = e.touches[0].pageY;
      ...
      touchMove(e) {
				...
				//上拉时，固定顶部
        var fixedTop = this.fixedTop
        if(moveY < 0){
          //console.log('上拉scrollTop:',document.body.scrollTop)
          //console.log('上拉fixedTop:',fixedTop)
          if(bodyScrollTop < fixedTop){
            return;
          }
        }
        if(moveY > 1){
          //console.log('下拉scrollTop:',document.body.scrollTop)
          if(bodyScrollTop > 0){
            return;
          }
        }
