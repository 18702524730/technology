    ...
    <div class="header">
      ...
      <div class="boxes test">
        <form>
          ...
          <div class="form_item">
            <label><i>*</i>近三年企业总研发费用或管理费用(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.researchCost" maxlength="10" placeholder="请输入近三年企业总研发费用或管理费用" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>2017年企业销售收入(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.saleCost1" maxlength="10" placeholder="请输入2017年企业销售收入" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>2017年企业所有者权益(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.income1" maxlength="10" placeholder="请输入2017年企业所有者权益" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>2016年企业销售收入(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.saleCost2" maxlength="10" placeholder="请输入2016年企业销售收入" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>2016年企业所有者权益(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.income2" maxlength="10" placeholder="请输入2016年企业所有者权益" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>2015年企业销售收入(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.saleCost3" maxlength="10" placeholder="请输入2015年企业销售收入" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>2015年企业所有者权益(万元)</label>
            <div class="form_item_content">
              <input type="text" name="name" v-model="dform.income3" maxlength="10" placeholder="请输入2015年企业所有者权益" />
            </div>
          </div>
          <div class="form_item">
            <label><i>*</i>企业纳税方式</label>
            <div class="form_item_content noborder">
              <el-radio v-model="dform.taxesType" label="1">查账征收</el-radio>
              <el-radio v-model="dform.taxesType" label="2">额定征收</el-radio>
            </div>
          </div>
...
<script>
  ...
  const addUrl = cartUrl + '/hightech/addHightechNew.htm';
	...
	export default {
      ...
      checkValid(){
        ...
        if (!this.dform.saleCost1 || !this.dform.saleCost2 || !this.dform.saleCost3) {
          this.showToast = true;
          this.tip = '近三年销售收入不能为空'
          return;
        }
        if (!/(^0(\.\d{1,2})?$)|(^[1-9]\d*(\.\d{1,2})?$)/.test(this.dform.saleCost1) || !/(^0(\.\d{1,2})?$)|(^[1-9]\d*(\.\d{1,2})?$)/.test(this.dform.saleCost2) || !/(^0(\.\d{1,2})?$)|(^[1-9]\d*(\.\d{1,2})?$)/.test(this.dform.saleCost3)) {
          this.showToast = true;
          this.tip = '销售收入请输入数字，可精确到小数点后两位'
          return;
        }
        if (!this.dform.income1 || !this.dform.income2 || !this.dform.income3) {
          this.showToast = true;
          this.tip = '近三年企业所有者权益不能为空'
          return;
        }
        if (!/(^0(\.\d{1,2})?$)|(^[1-9]\d*(\.\d{1,2})?$)/.test(this.dform.income1) || !/(^0(\.\d{1,2})?$)|(^[1-9]\d*(\.\d{1,2})?$)/.test(this.dform.income2) || !/(^0(\.\d{1,2})?$)|(^[1-9]\d*(\.\d{1,2})?$)/.test(this.dform.income3)) {
          this.showToast = true;
          this.tip = '所有者权益请输入数字，可精确到小数点后两位'
          return;
        }
