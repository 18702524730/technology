<template>
  <div class="big_scholar" :class="{'autoHeight': selectBox}">
    ...
    <div id="content_detail" style="display: block;">
      <div class="bigData-wp" style="background: #fff">
        <!--石墨烯 scholar graph-->
        <div style="position: relative;height: 100%">
          <div id="G_s_graph" class="bd-chart" _echarts_instance_="ec_1539329932904" style="-webkit-tap-highlight-color: transparent; user-select: none; position: relative;"><div style="position: relative; overflow: hidden; width: 375px; height: 667px; padding: 0px; margin: 0px; border-width: 0px;"><canvas data-zr-dom-id="zr_0" width="750" height="1334" style="position: absolute; left: 0px; top: 0px; width: 375px; height: 667px; user-select: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0); padding: 0px; margin: 0px; border-width: 0px;"></canvas></div><div></div></div>
...
<script>
  ...
  import scholarJs from 'assets/js/scholar.js';
  ...
  export default {
    ...
    mounted(){
      ...
      // this.wxShare('相关学者', location.href.split('#')[0], '', '相关学者', url); //微信分享  在mian.js里Vue的原型里已定义
    ...
    methods: {
      ...
      searchFn () {
        scholarJs.initFn(this.keyword, this.unitCode, this.provinceCode);
      }
    },
    watch: {
      unitCode (n, o) {
...
<style lang="scss"  >
  .big_scholar {
    ...
    .bd-filter {
      ...
      .weui-cells {
        width: 150px; height: 32px; line-height: 32px; box-sizing:border-box; padding-left: 10px; background: #F2F6FF; margin: 0; border-radius:5px; padding: 0; 
        .vux-label {font-size: 12px; color: #666;}
        .vux-cell-value {font-size: 12px; color: #333;}
        .vux-cell-primary {height: 32px; line-height: 32px; min-width: 50px;}
