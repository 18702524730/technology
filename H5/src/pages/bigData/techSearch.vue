<template>
  <div class="techSearch">
    <!-- 搜索框 -->
    <search-box :content.sync="keyword" placeholder="请输入科技关键词，如'石墨烯'" :focus.sync="isFocus" @search="searchFn" @suggest="suggestFn" />
...
<script>
  ...
  export default {
    ...
    methods: {
      // 搜索
      searchFn (data) {
        this.$router.replace({path: '/bigData?keyword=' + data})
      },
      // 模糊搜索
      async suggestFn (data) {
        ...
        this.keyword = data;
        if (!data) {return}
        this.$http.get(`${itechUrl}/api/h5/suggestField?keyword=${data}`)
        .then( resp =>{
...
<style lang="scss">
  .techSearch {
    ...
    .pub_navbar{position: relative;z-index: 3;}
    .tab_panel{}
    .patent_data{
