<template>
    ...
    <form class="form_box" :class="{'fixed': fixedBtn}" id="form_box">
      <!-- <p class="boxes_in_til" v-if="!isRequired && hasRequrieKey">必填数据</p> -->
      ...
      <div class="form_item" id="companyName" v-if="showAll || showCompanyName">
        <label><i>*</i>企业名称:</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.companyName" placeholder="请输入企业名称" disabled />
        </div>
      </div>
      <div class="form_item" id="contact" v-if="showAll || showKey('contact')">
        <label><i>*</i>联系人:</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.contact" placeholder="请输入联系人" disabled />
        </div>
      </div>
      <div class="form_item" id="contactNumber" v-if="showAll || showKey('contactNumber')">
        <label><i>*</i>联系人手机号:</label>
        <div class="form_item_content">
      ...
      <div class="form_item" id="regAddress" v-if="showAll || showKey('regAddress')">
        <label><i>*</i>企业注册所在地:</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.regAddress" placeholder="请输入企业注册所在地" />
        </div>
      </div>
      <div class="form_item" id="street" v-if="dform.regAddress && dform.regAddress.indexOf('江干区') !== -1 && (showAll || showKey('street'))">
      ...
      <div class="form_item" id="regCapital" v-if="showAll || showKey('regCapital')">
        <label><i>*</i>注册资金(万元):</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.regCapital" maxlength="12" placeholder="请输入注册资金" />
        </div>
      </div>
      ...
      <div class="form_item" id="wantPledge" v-if="showAll || showKey('wantPledge')">
        ...
        <div class="form_item_content">
          ...
          <label @click="dform.wantPledge=2" class="radio_label"><span :class="{'hight': dform.wantPledge==2}"></span>没有</label>
        </div>
      </div>
      <div class="form_item" id="wantTechButt" v-if="showAll || showKey('wantTechButt')">
        ...
        <div class="form_item_content">
          ...
          <label @click="dform.wantTechButt=2" class="radio_label"><span :class="{'hight': dform.wantTechButt==2}"></span>没有</label>
        </div>
      </div>
      <div class="form_item" id="chargeName" v-if="showAll || showKey('chargeName')">
        <label for="chargeName1"><i>*</i>企业负责人(法人/分管副总):</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.chargeName" maxlength="14" placeholder="请输入企业负责人姓名" />
        </div>
      </div>
      <div class="form_item" id="chargeNumber" v-if="showAll || showKey('chargeNumber')">
        <label for="chargeNumber1"><i>*</i>企业负责人手机号:</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.chargeNumber" @change="phoneChange(dform.chargeNumber)" maxlength="11" placeholder="请输入企业负责人手机号" />
        </div>
      </div>
      <div class="form_item" id="chargeWx" v-if="showAll || showKey('chargeWx')">
        <label for="chargeWx1"><i>*</i>企业负责人微信号:</label>
        <!-- <div class="form_item_content checkbox_row">
          <el-checkbox v-model="phoneIsWxAccount">手机号就是微信号</el-checkbox>
        </div> -->
        <div class="form_item_content">
          <input type="text" v-model="dform.chargeWx" maxlength="20" placeholder="请输入企业负责人微信号" />
        </div>
      </div>
      
      <template v-if="hasRequrieKey && !isRequired && !showMore && !showCompanyName">
        <div class="more_items" @click="showMore=true">更多选填项</div>
      </template>
      <template v-else>
      <p class="boxes_in_til mt40" v-if="!isRequired">补充信息(选填)：</p>

      <div class="form_item" id="apply2019Status" v-if="showAll || showKey('apply2019Status')">
        <label><i v-if="isRequired">*</i>国家高新技术企业认定计划:</label>
        <div class="form_item_content noborder" style="padding-bottom: 0">
          <div class="applyStatus">
            <label v-if="hideRepeat" @click="dform.apply2019Status=1" class="radio_label"><span :class="{'hight': dform.apply2019Status==1}"></span>新认定</label>
            <label v-if="!hideRepeat || apply2019StatusqEqual2" @click="dform.apply2019Status=2" class="radio_label"><span :class="{'hight': dform.apply2019Status==2}"></span>复评</label>
            <label v-if="hideRepeat" @click="dform.apply2019Status=7" class="radio_label"><span :class="{'hight': dform.apply2019Status==7}"></span>暂无意向</label>
          </div>
          <div v-if="hideRepeat && dform.apply2019Status && dform.apply2019Status != 7">
            <a
          ...
          <div v-if="!hideRepeat && dform.apply2019Status && dform.apply2019Status != 7">
            ...
            :key="index"
            :class="['platformLabel', index === applyYearIndex ? 'activeLabel' : '']"
            v-if="dform.applyYear == item"
            @click="clickApplyYear(item, index)"
            href="javascript:void(0)">{{item}}</a>
          </div>
        </div>
      </div>
      <div class="form_item" v-if="showAll || showKey('sale2016') || showKey('sale2017') || showKey('sale2018')">
        <label class="hight"><i v-if="isRequired">*</i>企业近三年销售收入(万元):</label>
        <div class="form_item_content fle" id="sale2016" v-if="showAll || showKey('sale2016')">
        ...
        <div class="form_item_content fle" id="sale2017" v-if="showAll || showKey('sale2017')">
        ...
        <div class="form_item_content fle" id="sale2018" v-if="showAll || showKey('sale2018')">
          <p>2018年</p>
          <input type="text" v-model="dform.sale2018" maxlength="15" placeholder="请输入2018年销售收入" />
        </div>
      </div>
      <div class="form_item" id="salePrice" v-if="showAll || showKey('research2016Cost') || showKey('research2017Cost') || showKey('research2018Cost')">
        <label class="hight"><i v-if="isRequired">*</i>企业近三年研发费用(万元):</label>
        <div class="form_item_content fle" id="research2016Cost" v-if="showAll || showKey('research2016Cost')">
          <p :style="{'color': regYear == '2017' || regYear == '2018' ? '#bbb' : '#333'}">2016年</p>
          <input
          type="text"
        ...
        <div class="form_item_content fle" id="research2017Cost" v-if="showAll || showKey('research2017Cost')">
          <p :style="{'color': regYear == '2018' ? '#bbb' : '#333'}">2017年</p>
        ...
        <div class="form_item_content fle" id="research2018Cost" v-if="showAll || showKey('research2018Cost')">
          <p>2018年</p>
          <input type="text" v-model="dform.research2018Cost" placeholder="请输入2018年研发费用" />
        </div>
      </div>

      <div class="form_item" v-if="showAll || showKey('inventionAuthPatentNum') || showKey('utilityModelPatentNum') || showKey('industrialDesignPatentNum') || showKey('softCopyrightNum') || showKey('workCopyrightNum') || showKey('icDesignNum') || showKey('effectiveTmNum')">
        <label class="hight"><i v-if="isRequired">*</i>知识产权:</label>
        <div class="form_item_content fle2" id="inventionAuthPatentNum" v-if="showAll || showKey('inventionAuthPatentNum')">
          <p>发明授权专利</p>
          <input type="text" v-model="dform.inventionAuthPatentNum" :maxlength="6" placeholder="请输入发明授权专利数量" />
        </div>
        <div class="form_item_content fle2" id="utilityModelPatentNum" v-if="showAll || showKey('utilityModelPatentNum')">
          <p>实用新型专利</p>
          <input type="text" v-model="dform.utilityModelPatentNum" :maxlength="6" placeholder="请输入实用新型专利数量" />
        </div>
        <div class="form_item_content fle2" id="industrialDesignPatentNum" v-if="showAll || showKey('industrialDesignPatentNum')">
          <p>外观设计专利</p>
          <input type="text" v-model="dform.industrialDesignPatentNum" :maxlength="6" placeholder="请输入专利数量" />
        </div>
        <div class="form_item_content fle2" id="softCopyrightNum" v-if="showAll || showKey('softCopyrightNum')">
          <p>软件著作权</p>
          <input type="text" v-model="dform.softCopyrightNum" :maxlength="6" placeholder="请输入软件著作权数量" />
        </div>
        <div class="form_item_content fle2" id="workCopyrightNum" v-if="showAll || showKey('workCopyrightNum')">
          <p>作品著作权</p>
          <input type="text" v-model="dform.workCopyrightNum" :maxlength="6" placeholder="请输入作品著作权数量" />
        </div>
        <div class="form_item_content fle4" id="icDesignNum" v-if="showAll || showKey('icDesignNum')">
          <p>集成电路布图设计专有权</p>
          <input type="text" v-model="dform.icDesignNum" :maxlength="6" placeholder="请输入专有权数量" />
        </div>
        <div class="form_item_content fle2" id="effectiveTmNum" v-if="showAll || showKey('effectiveTmNum')">
          <p>有效商标数量</p>
          <input type="text" v-model="dform.effectiveTmNum" :maxlength="6" placeholder="请输入有效商标数量" />
        </div>
      </div>

      <div id="intellectualPropertyIso" class="form_item" v-if="showAll || showKey('intellectualPropertyIso')">
        <label>知识产权贯标</label>
        <div class="form_item_content noborder">
          <label @click="dform.intellectualPropertyIso=4" class="radio_label"><span :class="{'hight': dform.intellectualPropertyIso==4}"></span>是</label>
          <label @click="dform.intellectualPropertyIso=5" class="radio_label"><span class="radio_span" :class="{'hight': dform.intellectualPropertyIso==5}"></span>否</label>
        </div>
      </div>

      <div class="form_item" v-if="showAll || showKey('techTransformNum2016') || showKey('techTransformNum2017') || showKey('techTransformNum2018')">
        <label class="hight"><i v-if="isRequired">*</i>科技成果转化数量:</label>
        <div class="form_item_content fle" id="techTransformNum2016" v-if="showAll || showKey('techTransformNum2016')">
          <p :style="{'color': regYear == '2017' || regYear == '2018' ? '#bbb' : '#333'}">2016年</p>
          <input type="text" :disabled="regYear == '2017' || regYear == '2018'" v-model="dform.techTransformNum2016" @keyup="dform.techTransformNum2016 = dform.techTransformNum2016.slice(0, 12)" :placeholder="regYear == '2017' || regYear == '2018' ? transPlaceHolder : '请输入2016年转化数量'" />
        </div>
        <div class="form_item_content fle" id="techTransformNum2017" v-if="showAll || showKey('techTransformNum2017')">
          <p :style="{'color': regYear == '2018' ? '#bbb' : '#333'}">2017年</p>
          <input type="text" @keyup="dform.techTransformNum2017 = dform.techTransformNum2017.slice(0, 12)" :disabled="regYear == '2018'" v-model="dform.techTransformNum2017" :placeholder=" regYear == '2018' ? transPlaceHolder : '请输入2017年转化数量'" />
        </div>
        <div class="form_item_content fle" id="techTransformNum2018" v-if="showAll || showKey('techTransformNum2018')">
          <p>2018年</p>
          <input type="text" v-model="dform.techTransformNum2018" maxlength="15" placeholder="请输入2018年转化数量" />
        </div>
      </div>
      <div class="form_item" v-if="showAll || showKey('companyNum') || showKey('researchNum') || showKey('doctorSeniorNum') || showKey('middleTitleNum') || showKey('doctorNum') || showKey('masterNum') || showKey('collegeNum') || showKey('juniorNum') || showKey('companyRoundNum')  || showKey('researchRoundNum') || showKey('middleRoundNum') || showKey('collegeAboveRoundNum')">
        <label class="hight"><i v-if="isRequired">*</i>企业人员情况:</label>
        <div class="form_item_content fle2" id="companyRoundNum" v-if="showAll || showKey('companyRoundNum')">
          <p>企业人员概数</p>
          <input type="text" v-model="dform.companyRoundNum" :maxlength="9" placeholder="请输入企业人员概数" />
        </div>
        <div class="form_item_content fle2" id="researchRoundNum" v-if="showAll || showKey('researchRoundNum')">
          <p>研发人员概数</p>
          <input type="text" v-model="dform.researchRoundNum" :maxlength="9" placeholder="请输入研发人员概数" />
        </div>
        <div class="form_item_content fle5" id="middleRoundNum" v-if="showKey('middleRoundNum')">
          <p>本科及中级以上概数</p>
          <input type="text" v-model="dform.middleRoundNum" :maxlength="9" placeholder="请输入本科及中级以上概数" />
        </div>
        <label style="margin-top: 0.85rem" v-if="showAll || showKey('doctorSeniorNum') || showKey('middleTitleNum') || showKey('doctorNum') || showKey('masterNum') || showKey('collegeNum') || showKey('juniorNum') || showKey('collegeAboveRoundNum')">其中:</label>
        <div class="form_item_content fle5" id="collegeAboveRoundNum" v-if="showKey('collegeAboveRoundNum')">
          <p>本科及以上学历人数</p>
          <input type="text" v-model="dform.collegeAboveRoundNum" :maxlength="9" placeholder="请输入本科及以上学历人数" />
        </div>
        <div class="form_item_content fle5" id="doctorSeniorNum" v-if="showAll || showKey('doctorSeniorNum')">
          <p>博士或高工人数</p>
          <input type="text" v-model="dform.doctorSeniorNum" :maxlength="9" placeholder="请输入博士或高工人数" />
        </div>
        <div class="form_item_content fle5" id="middleTitleNum" v-if="showAll || showKey('middleTitleNum')">
          <p>中级职称人数</p>
          <input type="text" v-model="dform.middleTitleNum" :maxlength="9" placeholder="请输入中级职称人数" />
        </div>
        <!-- <div class="form_item_content fle2" id="doctorNum" v-if="showAll || showKey('doctorNum')">
          <p>博士</p>
          <input type="text" v-model="dform.doctorNum" :maxlength="9" placeholder="请输入博士数量" />
        </div>
        <div class="form_item_content fle2" id="masterNum" v-if="showAll || showKey('masterNum')">
          <p>硕士</p>
          <input type="text" v-model="dform.masterNum" :maxlength="9" placeholder="请输入硕士数量" />
        </div>
        <div class="form_item_content fle2" id="collegeNum" v-if="showAll || showKey('collegeNum')">
          <p>本科</p>
          <input type="text" v-model="dform.collegeNum" :maxlength="9" placeholder="请输入本科数量" />
        </div>
        <div class="form_item_content fle2" id="juniorNum" v-if="showAll || showKey('juniorNum')">
          <p>大专</p>
          <input type="text" v-model="dform.juniorNum" :maxlength="9" placeholder="请输入大专数量" />
        </div> -->
      </div>
      <div class="form_item" id="researchOrgLevel" v-if="showAll || showKey('researchOrgLevel')">
        <label><i v-if="isRequired">*</i>是否设有研发机构或平台:</label>
        <div class="form_item_content noborder">
      ...
      <div class="form_item" id="researchArea" v-if="showAll || showKey('researchArea')">
        <label for="researchArea1"><i v-if="isRequired">*</i>科研用房面积(平方米):</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.researchArea" maxlength="9" placeholder="请输入科研用房面积" />
        </div>
      </div>
      <div class="form_item" id="equipment" v-if="showAll || showKey('equipment')">
        <label for="equipment1"><i v-if="isRequired">*</i>企业技术开发仪器设备原值(万元):</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.equipment" maxlength="9" placeholder="请输入企业技术开发仪器设备原值" />
        </div>
      </div>
      <div class="form_item" id="passedSystem" v-if="showAll || showKey('passedSystem')">
        <label><i v-if="isRequired">*</i>已通过的体系认证（可多选）:</label>
        <!-- <div class="form_item_content gm">
          <popup-picker :title="''" :data="passedSystemList" v-model="selectObj.passedSystem" @on-change="onChange(selectObj.passedSystem, 'passedSystem')" placeholder="请选择已通过的体系认证"></popup-picker>
          <img src="~assets/img/index/right.png" alt="">
        </div> -->
        <div class="form_item_content">
          <p :class="['text', passedSystem ? '' : 'pl']" @click="handleSelectPro('hasPassedSystemState')">{{passedSystem ? passedSystem : '请选择已通过的体系认证'}}</p>
          <img src="~assets/img/index/right.png" @click="handleSelectPro('hasPassedSystemState')" alt="">
          <div v-transfer-dom>
            <popup v-model="hasPassedSystemState" position="bottom" max-height="50%">

              <check-list :options="passedSystemList" ref="proObject" v-model="passedSystemItem" label-position="left"  @on-change="change(passedSystemItem, 'passedSystem')"></check-list>
              <div class="blank"></div>
            </popup>
            <button v-show="hasPassedSystemState" :class="['selectBtn' ,hasPassedSystemState ? 'showBtn' : '']" @click="confirmPro" type="button"> 确定 </button>
          </div>
        </div>
      </div>
      <div class="form_item" id="isExcellenceManage" v-if="showAll || showKey('isExcellenceManage')">
        <label><i v-if="isRequired">*</i>是否引入卓越绩效管理模式:</label>
        <div class="form_item_content noborder">
          <label @click="dform.isExcellenceManage=0" class="radio_label"><span :class="{'hight': dform.isExcellenceManage===0}"></span>是</label>
          <label @click="dform.isExcellenceManage=1" class="radio_label"><span :class="{'hight': dform.isExcellenceManage===1}"></span>否</label>
        </div>
      </div>

      <div class="form_item" id="standardSituation" v-if="showAll || showKey('standardSituation')">
        <label><i v-if="isRequired">*</i>标准参与制定情况:</label>
        <div class="form_item_content gm">
          <popup-picker :title="''" :data="standardSituationList" v-model="selectObj.standardSituation" @on-change="onChange(selectObj.standardSituation, 'standardSituation')" placeholder="请选择标准参与制定情况"></popup-picker>
          <img src="~assets/img/index/right.png" alt="">
        </div>
      </div>

      <div class="subtil" v-if="showAll || showKey('certifiedLab') || showKey('awards') || showKey('infoInvestment') || showKey('outerExpertNum') || showKey('isAccident')">加分项</div>
      <div class="form_item" id="certifiedLab" style="margin-top: -0.5rem;" v-if="showAll || showKey('certifiedLab')">
        <label for="certifiedLab1"><i v-if="isRequired">*</i>通过省、国家（国际组织）认证的实验室和检测机构情况:</label>
        <div class="form_item_content has_extarea">
          <textarea v-model="dform.certifiedLab" maxlength="500" placeholder="请输入通过省、国家（国际组织）认证的实验室和检测机构情况"></textarea>
          <i class="textarea_num">{{dform.certifiedLab ? dform.certifiedLab.length : 0}}/500</i>
        </div>
      </div>
      <div class="form_item" id="awards" v-if="showAll || showKey('awards')">
        <label for="awards1"><i v-if="isRequired">*</i>近三年获得市级及以上自然科学、技术发明、科技进步奖项目情况:</label>
        <div class="form_item_content has_extarea">
          <textarea v-model="dform.awards" maxlength="500" placeholder="请输入近三年获得市级及以上自然科学、技术发明、科技进步奖项目情况"></textarea>
          <i class="textarea_num">{{dform.awards ? dform.awards.length : 0}}/500</i>
        </div>
      </div>
      <div class="form_item" id="infoInvestment" v-if="showAll || showKey('infoInvestment')">
        <label for="infoInvestment1"><i v-if="isRequired">*</i>近三年企业信息化建设投入(万元):</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.infoInvestment" maxlength="9" placeholder="请输入近三年企业信息化建设投入" />
        </div>
      </div>
      <div class="form_item" id="outerExpertNum" v-if="showAll || showKey('outerExpertNum')">
        <label for="outerExpertNum1"><i v-if="isRequired">*</i>中心从事研发工作的外部专家数:</label>
        <div class="form_item_content">
          <input type="text" v-model="dform.outerExpertNum" maxlength="9" placeholder="请输入中心从事研发工作的外部专家数" />
        </div>
      </div>
      <div class="form_item" id="isAccident" v-if="showAll || showKey('isAccident')">
        <label><i v-if="isRequired">*</i>近三年无重大环境污染事故、消防安全生产事故、偷税漏税和知识产权违法行为:</label>
        <div class="form_item_content noborder">
          <label @click="dform.isAccident=0" class="radio_label"><span :class="{'hight': dform.isAccident===0}"></span>是</label>
          <label @click="dform.isAccident=1" class="radio_label"><span :class="{'hight': dform.isAccident===1}"></span>否</label>
        </div>
      </div>
      </template>
...
<script>
      ...
      init() {
        ...
        this.whiteKeyNameList.some((item) => {
          if (this.requrieKeys.includes(item)) {
            if (item === 'street') {
              if (dform.regAddress && dform.regAddress.indexOf('江干区') !== -1) {
  ...
  　　},
      ...
      checkValid(){
        ...
        if (isNeedCheck('chargeName') && !dform.chargeName){
          this.showToast = true;
          this.custormAnchor('chargeName')
          this.tip = '请输入企业负责人姓名';
          return;
        }
        if (isNeedCheck('chargeNumber') && !dform.chargeNumber){
          this.showToast = true;
          this.custormAnchor('chargeNumber')
          this.tip = '请输入企业负责人手机号';
          return;
        }
        if (isNeedCheck('chargeNumber') && !/^(1[3-9])\d{9}$/.test(dform.chargeNumber)) {
          this.showToast = true;
          this.custormAnchor('chargeNumber')
          this.tip = '请输入正确的企业负责人手机号'
          return;
        }
        if (isNeedCheck('chargeWx') && !dform.chargeWx){
          this.showToast = true;
          this.custormAnchor('chargeWx')
          this.tip = '请输入企业负责人微信号';
          return;
        }

        //以下为选填项
        if (isNeedCheck('apply2019Status') && !dform.apply2019Status){
          this.showToast = true;
          this.tip = '请选择国家高新技术企业认定计划';
        ...
        if (parseInt(this.regYear) < 2019) {
          if(isNeedCheck('techTransformNum2018') && !dform.techTransformNum2018 && dform.techTransformNum2018 !== 0){
            this.showToast = true;
            this.custormAnchor('techTransformNum2018')
            this.tip = '请输入企业2018年科技成果转化数量'
            return;
          }
          if (parseInt(this.regYear) < 2018) {
            if(isNeedCheck('techTransformNum2017') && !dform.techTransformNum2017 && dform.techTransformNum2017 !== 0){
              this.showToast = true;
              this.custormAnchor('techTransformNum2017')
              this.tip = '请输入企业2017年科技成果转化数量'
              return;
            }
          }
          if (parseInt(this.regYear) < 2017) {
            if(isNeedCheck('techTransformNum2016') && !dform.techTransformNum2016 && dform.techTransformNum2016 !== 0){
              this.showToast = true;
              this.custormAnchor('techTransformNum2016')
              this.tip = '请输入企业2016年科技成果转化数量'
              return;
            }
          }
        }

        if (dform.techTransformNum2016 && (parseFloat(dform.techTransformNum2016) < 0 || !/(^[1-9][0-9]{0,8}$)|(^0$)/.test(dform.techTransformNum2016))) {
          this.showToast = true;
          this.custormAnchor('techTransformNum2016')
          this.tip = '2016年科技成果转化数量请输入正数(最多九位)';
          return;
        }
        if (dform.techTransformNum2017 && (parseFloat(dform.techTransformNum2017) < 0 || !/(^[1-9][0-9]{0,8}$)|(^0$)/.test(dform.techTransformNum2017))) {
          this.showToast = true;
          this.custormAnchor('techTransformNum2017')
          this.tip = '2017年科技成果转化数量请输入正数(最多九位)';
          return;
        }
        if (dform.techTransformNum2018 && (parseFloat(dform.techTransformNum2018) < 0 || !/(^[1-9][0-9]{0,8}$)|(^0$)/.test(dform.techTransformNum2018))) {
          this.showToast = true;
          this.custormAnchor('techTransformNum2018')
          this.tip = '2018年科技成果转化数量请输入正数(最多九位)';
          return;
        }
        ...
        /*if(isNeedCheck('doctorNum') && !dform.doctorNum && dform.doctorNum !== 0){
          this.showToast = true;
          this.custormAnchor('doctorNum')
          this.tip = '请输入博士数量'
          return;
        }
        if (isNeedCheck('doctorNum') && dform.doctorNum && (parseFloat(dform.doctorNum) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.doctorNum))){
          this.showToast = true;
          this.custormAnchor('doctorNum')
          this.tip = '博士数量请输入九位以内非负整数';
          return;
        }
        if (isNeedCheck('doctorNum') && dform.companyRoundNum && (dform.doctorNum-0 > dform.companyRoundNum)){
          this.showToast = true;
          this.custormAnchor('doctorNum')
          this.tip = '博士数量不能大于企业人员概数';
          return;
        }
        if(isNeedCheck('masterNum') && !dform.masterNum && dform.masterNum !== 0){
          this.showToast = true;
          this.custormAnchor('masterNum')
          this.tip = '请输入硕士数量'
          return;
        }
        if (isNeedCheck('masterNum') && dform.masterNum && (parseFloat(dform.masterNum) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.masterNum))){
          this.showToast = true;
          this.custormAnchor('masterNum')
          this.tip = '硕士数量请输入九位以内非负整数';
          return;
        }
        if (isNeedCheck('masterNum') && dform.companyRoundNum && (dform.masterNum-0 > dform.companyRoundNum)){
          this.showToast = true;
          this.custormAnchor('masterNum')
          this.tip = '硕士数量不能大于企业人员概数';
          return;
        }
        if(isNeedCheck('collegeNum') && !dform.collegeNum && dform.collegeNum !== 0){
          this.showToast = true;
          this.custormAnchor('collegeNum')
          this.tip = '请输入本科数量'
          return;
        }
        if (isNeedCheck('collegeNum') && dform.collegeNum && (parseFloat(dform.collegeNum) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.collegeNum))){
          this.showToast = true;
          this.custormAnchor('collegeNum')
          this.tip = '本科数量请输入九位以内非负整数';
          return;
        }
        if (isNeedCheck('collegeNum') && dform.companyRoundNum && (dform.collegeNum-0 > dform.companyRoundNum)){
          this.showToast = true;
          this.custormAnchor('collegeNum')
          this.tip = '本科数量不能大于企业人员概数';
          return;
        }
        if(isNeedCheck('juniorNum') && !dform.juniorNum && dform.juniorNum !== 0){
          this.showToast = true;
          this.custormAnchor('juniorNum')
          this.tip = '请输入大专数量'
          return;
        }
        if (isNeedCheck('juniorNum') && dform.juniorNum && (parseFloat(dform.juniorNum) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.juniorNum))){
          this.showToast = true;
          this.custormAnchor('juniorNum')
          this.tip = '大专数量请输入九位以内非负整数';
          return;
        }
        if (isNeedCheck('juniorNum') && dform.companyRoundNum && (dform.juniorNum-0 > dform.companyRoundNum)){
          this.showToast = true;
          this.custormAnchor('juniorNum')
          this.tip = '大专数量不能大于企业人员概数';
          return;
        }*/

        if (isNeedCheck('researchOrgLevel') && !dform.researchOrgLevel){
          this.showToast = true;
          this.custormAnchor('researchOrgLevel')
          this.tip = '请选择机构或者平台级别'
          return;
        }

        if (dform.researchArea && (parseFloat(dform.researchArea) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.researchArea))){
          this.showToast = true;
          this.custormAnchor('researchArea')
          this.tip = '科研用房面积请输入九位以内非负整数';
          return;
        }

        if (dform.equipment && (parseFloat(dform.equipment) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.equipment))){
          this.showToast = true;
          this.custormAnchor('equipment')
          this.tip = '仪器设备原值请输入九位以内非负整数';
          return;
        }
        if (dform.infoInvestment && (parseFloat(dform.infoInvestment) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.infoInvestment))){
          this.showToast = true;
          this.custormAnchor('infoInvestment')
          this.tip = '近三年企业信息化建设投入请输入九位以内非负整数';
          return;
        }
        if (dform.outerExpertNum && (parseFloat(dform.outerExpertNum) < 0 || !/(^[1-9]\d{0,8}$)|(^0$)/.test(dform.outerExpertNum))){
          this.showToast = true;
          this.custormAnchor('infoInvestment')
          this.tip = '从事研发工作的外部专家数请输入九位以内非负整数';
          return;
        }
...
@import  "~assets/css/cssshake.css";
    ...
    .form_box{
      ...
      .boxes_in_til{
        font-size: 30px;line-height: 42px;font-weight:600;padding-left: 40px;
      }
      ...
      .form_item{
        ...
        >label{
          ...
          &.hight{color: #333;font-weight: 600;}
        }
        .form_item_content{
          ...
          &.fle4{display: flex;flex-direction: row;justify-content: flex-start;
            p{width: 360px;font-size: 30px;color: #333;line-height: 42px;height: 42px;}
            input{width: 240px; line-height: 42px;height: 42px;}
          }
          &.fle5{display: flex;flex-direction: row;justify-content: flex-start;
            p{width: 300px;font-size: 30px;color: #333;line-height: 42px;height: 42px;}
            input{width: 300px; line-height: 42px;height: 42px;}
          }
          .radio_label{
      ...
      .footer_btn{
        text-align: center;padding: 40px 40px 0;font-size: 28px;color: #999;
    ...
    .fixed{
      .footer_btn{
        position: fixed;left: 0; bottom: 0; width: 100%;padding: 0;box-shadow:-2px -2px 10px 0px rgba(244,244,244,1);
        .button{border-radius: 0;font-size: 32px; height: 98px;line-height: 98px;}
      }
    }
