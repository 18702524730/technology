<template>
  <div class="pick_wrap" v-if="showList">
    ...
    <div class="pick_fixed" :class="{'move': showList, 'moveDown': !showList}">
      <a class="pick_item" v-for="(item, index) in list" :key="index" :href="'tel:'+item.title">{{ item.title }}</a>
...
<style lang="scss">
  ...
  .pick_fixed {
    width:100%; height: auto; background: #f7f7f7; position: fixed; left: 0; bottom: -350px; z-index: 10;
    ...
    .pick_item {
      width: 100%; height: 105px; line-height: 105px; border-bottom:1px solid #E5E5E5; text-align: center; font-size: 32px; color: #333; background: #fff;display: block;text-decoration: none;
