  ...
  <div class="gg-card-container">
  	...
  	<div class="card-wrap mt20">
  		...
  		<div class="card" :class="{'active': selectType==2}" @click="selectTypeFn(2)">
	    	<p title="新认定且符合“国家高新技术企业”申报条件的企业总数(家)">新认定且符合“国家高新技术企业”申报条件的企业总数(家)</p>
  ...
  methods: {
  	...
  	initData(type) {
  		...
  		if (this.isSuper) {
			...
			setTimeout(() => {
				this.show = true;
				let d = this.isSuper ? this.tData.street[0] : this.tData;
				this.options.xAxis.categories = [d.streetName]
				this.options.yAxis.title.text = this.tData.name
				this.options.series[0].data = [d.num];
				this.options.colors[0] = this.isSuper ? this.chartOptions.colors[0] : '#3576FF';
				this.options.title.text = '<i class="light'+ (this.isSuper ? ' light0' : '') +'">' + d.streetName + this.tData.chartTitle +'：' + (d.num || 0) + '家</i>'
			}, 1000)
  	},
  	initChartLine (type) {
  		this.show = false;
  		...
  		this.tData = { ...tData, ...this.getSomeData(type) }
  		console.log(this.tData)
  		let list = this.isSuper ? this.tData.street : this.tData;
    	...
    	if (this.isSuper) {
    		...
    		categories.push(list.streetName);
	    	data.push([list.low, list.high])
    	}
    	console.log(list)
  	...
  	pathTo(type) {
  		type = (type + '') || '0';
  		let path = '/home/nationalTech/list';
  		this.$router.push({
  			path,
  			query: {
	  			type
